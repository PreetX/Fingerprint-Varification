...
class FpMatchingEvaluator:
    ...
    def calculateResults(self):
        ...
        for th in range(101):
            ...
            #display
            print(f"threshold = {th}:false accept = {self.falseAccept[th]} false reject = {self.falseReject[th]}")
            self.outFile.write(f"threshold = {th}:false accept = {self.falseAccept[th]} false reject = {self.falseReject[th]}\n")

        eer = self.calculateEER()
        print("EER = ", eer)
        self.outFile.write(f"EER = {eer}\n")
        self.outFile.close()
